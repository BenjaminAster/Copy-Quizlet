...
document.onkeydown = function (evt) {
	...
	if (evt.altKey && evt.shiftKey) {
	...
	} else if ((typeof keysDown.w !== "undefined" ||
		...
		let tempActiveCellIndex
		// i = +i;
		console.log({ i });
		if (i >= editedVocArr.length) {
			rowIndex = (up) ? 0 : editedVocArr.length;
			tempActiveCellIndex = rowIndex * 2;
		} else {
			rowIndex += (up ? 0 : 1);
			tempActiveCellIndex = Math.floor((activeCellIndex + (up ? 0 : 2)) / 2) * 2;
		}
	...
	} else if (typeof keysDown.Tab !== "undefined" && typeof keysDown.Shift === "undefined") {
		if (activeCellIndex >= editedVocArr.length * 2 - 1) {
